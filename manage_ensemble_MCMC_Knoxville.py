...
postproc_only = True # True - do not re-run the ensemble

# Adjust the variables to compare against observations; need to contain all the observed variables, can have unused ones.
my_postproc_vars = ['H2OSOI','TLAI_pft','QVEGT_pft']
my_postproc_vars_pft = ['H2OSOI','TLAI_pft7','TLAI_pft13','QVEGT_pft7','QVEGT_pft13']
...
suffix = "_T0.00" # ambient condition to compare with ambient data
...
#Create case object
if (not UQ_only):  
    os.system(f"cp {PATH_URBAN}OLMT/pklfiles/{casename}.pkl_actual_ensemble_run {PATH_URBAN}OLMT/UQ_output/{casename}/{casename}.pkl")
    ...
    mycase.postproc_startyear=2024    #Starting year to postprocess/calibrate
...
if (not UQ_only):
    ...
    # save case
    mycase.create_pkl(outdir=mycase.OLMTdir+'/UQ_output/{casename}/')


# Read observed data: monthly 10cm soil moisture and tree transpiration
# Make sure the unit is the same as model output
# @Xiaoman Lu
